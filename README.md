...
Download the latest `GitHub.PR.Viewer-x.x.x-arm64.zip` from the [Releases](https://github.com/jwkim1993/pr-viewer/releases) page.

### First-Time Installation

This app is not signed with an Apple Developer certificate, so macOS Gatekeeper will block it on first launch. Use one of the following methods to open it.

**Option 1: Allow in System Settings (Recommended)**

1. Unzip the downloaded file and move `GitHub PR Viewer.app` to `/Applications`.
2. Double-click the app to launch it.
3. A warning dialog will appear saying the app "can't be opened because Apple cannot check it for malicious software." Click **OK**.
4. Open **System Settings > Privacy & Security**.
5. Scroll down to find a message about "GitHub PR Viewer." Click **Open Anyway**.
6. Click **Open** in the confirmation dialog.
7. The app will launch normally from now on.

**Option 2: Remove Quarantine via Terminal (Quickest)**

```bash
xattr -cr /Applications/GitHub\ PR\ Viewer.app
```

After running this command, the app will open without any warnings.

**Option 3: Right-Click to Open**

1. Right-click (or Control-click) `GitHub PR Viewer.app`.
2. Select **Open** from the context menu.
3. Click **Open** in the warning dialog.

> **Note**: This process is only required once. After the first successful launch, the app will open normally like any other application.

### Updating from a Previous Version

If you already have an older version installed:

1. **Quit** the running app completely (right-click the Dock icon > Quit, or use the system tray > Quit).
2. **Delete** the old `GitHub PR Viewer.app` from `/Applications`.
3. **Move** the new `GitHub PR Viewer.app` from the downloaded zip to `/Applications`.
4. **Remove quarantine** on the new version (required again for each new download):

```bash
xattr -cr /Applications/GitHub\ PR\ Viewer.app
```

5. Launch the app. All your settings (GitHub token, watched repos, AI API key, cached summaries, etc.) are preserved automatically since they are stored separately in `~/Library/Application Support/github-pr-viewer/`.

> **Tip**: If the app still shows the Gatekeeper warning after updating, run the `xattr -cr` command above or use the System Settings method again.
